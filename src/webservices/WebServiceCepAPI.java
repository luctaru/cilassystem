...
/**
 * Ferramenta de busca de CEP, veja o m�todo {@link WebServiceCepAPI#searchCep(String)} para
 * maiores informa��es.
 * <BR>
 * <BR>Constroi um objeto {@link WebServiceCepAPI} com os dados XML encapsulados, a partir
 * da chamada do m�todo estatico {@link WebServiceCepAPI#searchCep(String)}.
 * <BR>
 ...
 * <BR>
 * <BR>Exemplo de uso:
 * <BR><tt>{@link WebServiceCepAPI} cep = {@link WebServiceCepAPI}.searchCep("13345-325");
 * 
 ...
 */
public final class WebServiceCepAPI {
...
/* Classes Internas, que auxiliam na busca do CEP */
	...
	private enum Xml {
		CIDADE {
			@Override public void setCep(String text, WebServiceCepAPI webServiceCep) {
		 ...
		 * {@link WebServiceCepAPI} no seu parametro correspondente. Cada constante do enum
		 * conhece o seu parametro a passar.
		 * @param text {@link String} contendo o texto a ser inserido.
		 * @param webServiceCep {@link WebServiceCepAPI} referencia do objeto para inserir
		 * 		  o parametro text.
		 */
		public abstract void setCep(String text,WebServiceCepAPI webServiceCep);
	}
	/**
...
/* M�todos e variaveis estaticas, respons�veis pela busca do CEP */
	 ...
	 * @return {@link WebServiceCepAPI} contendo as informa��es da pesquisa.
	 */
	public static WebServiceCepAPI searchCep(String cep) {
		cep = cep.replaceAll( "\\D*", "" ); //To numeric digits only
		if (cep.length() > 8)
			cep = cep.substring(0, 8);
		WebServiceCepAPI loadCep = new WebServiceCepAPI(cep);
		try {
...
/* Campos internos de resultado da busca */
	 ...
	 * @param cep
	 */
    private WebServiceCepAPI(String cep) {
    	this.cep = cep;
    }
